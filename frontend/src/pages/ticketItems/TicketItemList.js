...
import { getAllTicketItems } from '../../api/services/ticketItem/ticketItem.api'; // Adjust the import path as necessary
...
const TicketItemList = () => {
 ...
 const filteredTicketItems = ticketItems.filter(ticketItem =>
   ticketItem.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
   ticketItem.type.toLowerCase().includes(searchTerm.toLowerCase()) ||
   ticketItem.ref.toLowerCase().includes(searchTerm.toLowerCase())
 );
      ...
      <div className="bg-white p-8 rounded-md w-full">
        <div className="flex items-center justify-between pb-6">
          ...
          <div className="lg:ml-40 ml-10 space-x-8">
            <Link to="/CreateTicketItem" className="bg-gray-600 px-4 py-2 rounded-md text-white font-semibold tracking-wide cursor-pointer">Create</Link>
          </div>
        </div>
        <div>
    ...
    {filteredTicketItems.map((ticketItem) => (
      <tr key={ticketItem.id}>
        <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
          <p className="text-gray-900 whitespace-no-wrap">{ticketItem.id}</p>
        </td>
        <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
          <p className="text-gray-900 whitespace-no-wrap">{ticketItem.name}</p>
        </td>
        <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
          <p className="text-gray-900 whitespace-no-wrap">{ticketItem.type}</p>
        </td>
        <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
          <p className="text-gray-900 whitespace-no-wrap">{ticketItem.ref}</p>
        </td>
        <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
          <p className="text-gray-900 whitespace-no-wrap">{ticketItem.createdAt? ticketItem.createdAt.split('T')[0] : 'No date available'}</p>
        </td>
                      <td className="px-5 py-5 border-b border-gray-200 bg-white text-sm">
                        <div className="flex justify-end">
                          <button className="bg-blue-700 px-4 py-2 rounded-md text-white font-semibold tracking-wide cursor-pointer">
                            <Link to={`/EditTicketItem/${ticketItem.id}`} className="flex items-center">
...
<button className="bg-red-600 px-4 py-2 rounded-md text-white font-semibold tracking-wide cursor-pointer ml-4">
  <Link to={`/DeleteTicketItem/${ticketItem.id}`} className="flex items-center">
